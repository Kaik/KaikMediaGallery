...
class GalleryAjaxController extends AbstractController
{
    ...
    public function modifyAction(Request $request, $id = null)
    {
        ...
        $options['action'] =  $this->get('router')->generate('kaikmediapagesmodule_galleryajax_modify', [], RouterInterface::ABSOLUTE_URL);
        ...
        if($mode == 'html'){
	       $template = $this->renderView('KaikmediaPagesModule:Gallery:new.form.html.twig', [
    ...
    public function getAction(Request $request, File $file)
    {
        ...
        $mode = 'html';
        if($mode == 'html'){
            $template = $this->renderView('KaikmediaPagesModule:Gallery:element.html.twig', [
                'file' => $file]);
        }else {
            $template['id'] = $file->getId();
            $template['name'] = $file->getName();
