 ...
 * @author Kaik
 */
class GalleryProviderBindingType extends AbstractHookType
{
...
//    "process_delete" => "processDelete"
        ...
        if (is_array($options) && array_key_exists('matchedEvents', $options)) {
            if (array_key_exists('display_view', $options['matchedEvents'])) {
                $builder->add('display', GalleryProviderDisplaySettingsType::class, [
                    'required'     => false,
                ]);
            }
       
            if (array_key_exists('form_edit', $options['matchedEvents'])) {
                $builder->add('edit', GalleryProviderEditSettingsType::class, [
                    'required'     => false,
                ]);
            } 
    ...
    {
        return "kmgallery_provider_gallery_binding";
    }
